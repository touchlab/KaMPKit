...
**Note:** We are using a Koin Multiplatform for KaMP Kit. This is a [3-rd version](https://doc.insert-koin.io/#/setup/koin_mp) of official Koin. It designed to work well with Kotlin/Native's state model. The difference is that all config is done in the main thread, and unless specified, all injecting is also done in the main thread. This is enforced in Native as well as Android to maintain consistency. You can freeze the objects you pul in there, but Koin will not freeze them unless told to do so. We'll be presenting this architecture to the Koin author after some experimentation.
